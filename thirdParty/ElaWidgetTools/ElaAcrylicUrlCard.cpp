...
ElaAcrylicUrlCard::ElaAcrylicUrlCard(QWidget *parent)
    : QPushButton(parent)
    , d_ptr(new ElaAcrylicUrlCardPrivate())
{
    ...
    d->_pCardPixmapSize = QSize((height() - d->_shadowBorderWidth) * 0.28,
                                (height() - d->_shadowBorderWidth) * 0.28);
    d->_pTitleSpacing = 10;
...
void ElaAcrylicUrlCard::paintEvent(QPaintEvent *event)
{
    ...
    if (!d->_pCardPixmap.isNull()) {
        ...
        if (d->_pCardPixMode == ElaCardPixType::PixMode::Ellipse) {
            path.addEllipse(QPointF(d->_pCardPixmapSize.width() / 2 + width / 7, height() / 4),
                            d->_pCardPixmapSize.width() / 2,
                            d->_pCardPixmapSize.height() / 2);
            painter.setClipPath(path);
            painter.drawPixmap(QRect(width / 7,
            ...
            painter.drawPixmap(width / 7 + d->_shadowBorderWidth,
                               height() / 4 - d->_pCardPixmapSize.height() / 2,
                               d->_pCardPixmapSize.width(),
                               d->_pCardPixmapSize.height(),
                               d->_pCardPixmap);
        } else if (d->_pCardPixMode == ElaCardPixType::PixMode::RoundedRect) {
            path.addRoundedRect(QRectF(width / 7,
    ...
    painter.drawText(QRect(width / 7,
                           height() / 4 + d->_pCardPixmapSize.height() / 2 + d->_pTitleSpacing,
                           width - width / 7,
                           height() / 3),
    ...
    painter.drawText(QRect(width / 7,
                           titleHeight + height() / 4 + d->_pCardPixmapSize.height() / 2
                               + d->_pTitleSpacing,
                           width - width / 7,
                           height() / 3),
                     Qt::AlignLeft | Qt::AlignTop | Qt::TextWordWrap,
    ...
    painter.drawText(width - iconFont.pixelSize() - 1,
                     height() - iconFont.pixelSize(),
                     QChar((unsigned short) ElaIconType::UpRightFromSquare));
    painter.restore();
}
