...
ElaMultiSelectComboBox::ElaMultiSelectComboBox(QWidget *parent)
    : QComboBox(parent)
    , d_ptr(new ElaMultiSelectComboBoxPrivate())
{
    ...
    connect(d->_comboView,
            &ElaComboBoxView::itemPressed,
            d,
            &ElaMultiSelectComboBoxPrivate::onItemPressed);
    connect(this,
            QOverload<int>::of(&ElaMultiSelectComboBox::currentIndexChanged),
            d,
            &ElaMultiSelectComboBoxPrivate::_refreshCurrentIndexs);
...
void ElaMultiSelectComboBox::setCurrentSelection(QString selection)
{
    ...
    for (int i = 0; i < this->count(); i++) {
        if (selection == itemText(i)) {
            ...
            d->_comboView->selectionModel()->select(index,
...
void ElaMultiSelectComboBox::setCurrentSelection(int index)
{
    ...
    d->_comboView->selectionModel()->select(currentIndex,
...
void ElaMultiSelectComboBox::paintEvent(QPaintEvent *e)
{
    ...
    QString currentText = painter.fontMetrics().elidedText(d->_currentText,
                                                           Qt::ElideRight,
                                                           foregroundRect.width() - 27
                                                               - width() * 0.05);
    ...
    if (view()->isVisible()) {
        // 展开
        painter.drawText(width() * 0.95 - iconFont.pixelSize(),
                         height() / 2 + iconFont.pixelSize() / 2 - 1,
                         QChar((unsigned short) ElaIconType::AngleUp));
    } else {
        // 未展开
        painter.drawText(width() * 0.95 - iconFont.pixelSize(),
                         height() / 2 + iconFont.pixelSize() / 2 - 1,
                         QChar((unsigned short) ElaIconType::AngleDown));
    }
    painter.restore();
}
...
void ElaMultiSelectComboBox::hidePopup()
{
    ...
    if (this->view()->underMouse()) {
        ...
        if (d->_isHidePopupAnimationFinished) {
            QWidget *container = this->findChild<QFrame *>();
            if (container) {
                QPropertyAnimation *opcaityAnimation = new QPropertyAnimation(container,
                                                                              "windowOpacity");
                ...
                viewPosAnimation->setEndValue(
                    QPoint(viewPos.x(), viewPos.y() - d->_comboView->height()));
