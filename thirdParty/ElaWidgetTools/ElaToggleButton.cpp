...
ElaToggleButton::ElaToggleButton(QWidget *parent)
    : QPushButton(parent)
    , d_ptr(new ElaToggleButtonPrivate())
{
    Q_D(ElaToggleButton);
    d->q_ptr = this;
    d->_initStyle();
...
void ElaToggleButton::paintEvent(QPaintEvent *event)
{
    ...
    if (d->_themeMode == ElaApplicationType::Light) {
        if (d->_isToggled) {
            ...
            painter.setBrush(d->_isPressed ? QColor(0x00, 0x7A, 0xD8)
                                           : (underMouse() ? QColor(0x00, 0x70, 0xC6)
                                                           : QColor(0x00, 0x66, 0xB9)));
        } else {
            painter.setPen(QPen(QColor(0xDF, 0xDF, 0xDF), 2));
            painter.setBrush(d->_isPressed ? QColor(0xF2, 0xF2, 0xF2)
                                           : (underMouse() ? QColor(0xF6, 0xF6, 0xF6)
    ...
    if (d->_themeMode == ElaApplicationType::Light) {
        palette.setColor(QPalette::ButtonText,
                         d->_isToggled ? QColor(0xFD, 0xFD, 0xFD) : QColor(0x45, 0x44, 0x41));
    } else {
        palette.setColor(QPalette::ButtonText,
                         d->_isToggled ? QColor(0x01, 0x01, 0x02) : QColor(0xFE, 0xFE, 0xFE));
    }
    setPalette(palette);
