...
ElaToggleSwitch::ElaToggleSwitch(QWidget *parent)
    : QWidget{parent}
    , d_ptr(new ElaToggleSwitchPrivate())
{
    Q_D(ElaToggleSwitch);
    d->q_ptr = this;
...
void ElaToggleSwitch::paintEvent(QPaintEvent *event)
{
    ...
    if (d->_themeMode == ElaApplicationType::Light) {
        painter.setPen(d->_isToggled ? QPen(QColor(0x00, 0x64, 0xB0), 2)
                                     : QPen(QColor(0xA5, 0xA5, 0xA7), 2));
        painter.setBrush(
            d->_isToggled ? QColor(0x00, 0x64, 0xB0)
                          : (underMouse() ? QColor(0xF0, 0xF0, 0xF0) : QColor(0xFD, 0xFD, 0xFD)));
    } else {
        painter.setPen(d->_isToggled ? Qt::NoPen : QPen(QColor(0xA1, 0xA1, 0xA1), 2));
        painter.setBrush(
            d->_isToggled ? QColor(0x4C, 0xA0, 0xE0)
                          : (underMouse() ? QColor(0x3E, 0x3E, 0x3E) : QColor(0x32, 0x32, 0x32)));
    }
    QPainterPath path;
    ...
    path.arcTo(QRectF(QPointF(d->_margin, d->_margin),
                      QSize(height() - d->_margin * 2, height() - d->_margin * 2)),
               90,
               180);
    ...
    if (d->_circleRadius == 0) {
        d->_circleRadius = this->isEnabled() ? (underMouse() ? height() * 0.35 : height() * 0.3)
                                             : height() * 0.3;
    }
    if (d->_isLeftButtonPress) {
        painter.drawEllipse(QPointF(d->_circleCenterX, height() / 2),
                            d->_circleRadius,
                            d->_circleRadius);
    } else {
        if (d->_isAnimationFinished) {
            painter.drawEllipse(QPointF(d->_isToggled ? width() - height() / 2 - d->_margin * 2
                                                      : height() / 2,
                                        height() / 2),
                                d->_circleRadius,
                                d->_circleRadius);
