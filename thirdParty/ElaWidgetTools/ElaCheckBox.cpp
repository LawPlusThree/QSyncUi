    ...
    : QCheckBox(parent)
{
    ...
    connect(ElaApplication::getInstance(),
            ...
            this,
            [=](ElaApplicationType::ThemeMode themeMode) { _themeMode = themeMode; });
}
...
void ElaCheckBox::paintEvent(QPaintEvent *event)
{
    ...
    if (_themeMode == ElaApplicationType::Light) {
        painter.setPen(isChecked()          ? QColor(0x00, 0x66, 0xB4)
                       : _isLeftButtonPress ? QColor(0xBF, 0xBF, 0xBF)
        ...
        painter.setBrush(isChecked()      ? _isLeftButtonPress ? QColor(0x3A, 0x7B, 0xAC)
                                            : _isHoverButton   ? QColor(0x42, 0x8B, 0xC3)
                                                               : QColor(0x4C, 0xA0, 0xE0)
                              : _isHoverButton ? QColor(0x48, 0x48, 0x48)
                                          : QColor(0x2D, 0x2D, 0x2D));
    }
    painter.drawRoundedRect(QRect(1, height() / 2 - height() * 0.4, height() * 0.8, height() * 0.8),
                            2,
                            2);
    painter.setPen(_themeMode == ElaApplicationType::Light ? Qt::black : Qt::white);
    painter.drawText(QRect(height() * 0.8 + _textLeftSpacing, 0, width(), height()),
                     Qt::AlignVCenter,
                     text());
