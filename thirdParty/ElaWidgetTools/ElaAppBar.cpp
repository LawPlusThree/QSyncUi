...
ElaAppBar::ElaAppBar(QWidget *parent)
    : QWidget{parent}
    , d_ptr(new ElaAppBarPrivate())
{
    Q_D(ElaAppBar);
    d->_buttonFlags = ElaAppBarType::RouteBackButtonHint | ElaAppBarType::StayTopButtonHint
                      | ElaAppBarType::ThemeChangeButtonHint | ElaAppBarType::MinimizeButtonHint
    ...
    // 展开导航栏
    connect(d->_navigationButton, &ElaIconButton::clicked, this, [this]() {
        Q_EMIT navigationButtonClicked();
    });
    ...
    connect(d->_themeChangeButton, &ElaIconButton::clicked, this, [this]() {
        Q_EMIT themeChangeButtonClicked();
    });
    connect(ElaApplication::getInstance(),
            &ElaApplication::themeModeChanged,
            this,
            [=](ElaApplicationType::ThemeMode themeMode) { d->_onThemeModeChange(themeMode); });
...
void ElaAppBar::setWindowButtonFlags(ElaAppBarType::ButtonFlags buttonFlags)
{
    ...
    d->_themeChangeButton->setVisible(
        d->_buttonFlags.testFlag(ElaAppBarType::ThemeChangeButtonHint));
...
void ElaAppBar::closeWindow()
{
    ...
    closeGeometryAnimation->setEndValue(
        QRect(windowRect.x() + (windowRect.width() - minmumWidth) / 2,
              windowRect.y() + (windowRect.height() / 2) - 145,
              minmumWidth,
              290));
