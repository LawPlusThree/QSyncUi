...
Q_PROPERTY_CREATE_Q_CPP(DirCard, int, BorderRadius)
Q_PROPERTY_CREATE_Q_CPP(DirCard, QString, Title);
Q_PROPERTY_CREATE_Q_CPP(DirCard, QString, SubTitle);
Q_PROPERTY_CREATE_Q_CPP(DirCard, int, TitlePixelSize);
Q_PROPERTY_CREATE_Q_CPP(DirCard, int, SubTitlePixelSize);
Q_PROPERTY_CREATE_Q_CPP(DirCard, int, TitleSpacing);
Q_PROPERTY_CREATE_Q_CPP(DirCard, QPixmap, CardPixmap);
Q_PROPERTY_CREATE_Q_CPP(DirCard, QSize, CardPixmapSize);
Q_PROPERTY_CREATE_Q_CPP(DirCard, int, CardPixmapBorderRadius)
Q_PROPERTY_CREATE_Q_CPP(DirCard, ElaCardPixType::PixMode, CardPixMode);

DirCard::DirCard(QWidget *parent)
    : QPushButton{parent}
    , d_ptr(new DirCardPrivate())
{
    Q_D(DirCard);
    d->q_ptr = this;
    d->_pBorderRadius = 6;
    ...
    d->_pCardPixmapSize = QSize((height() - d->_shadowBorderWidth) * 0.8,
                                (height() - d->_shadowBorderWidth) * 0.8);
    d->_pTitleSpacing = 3;
...
void DirCard::setCardPixmapSize(int width, int height)
{
    Q_D(DirCard);
    d->_pCardPixmapSize = QSize(width, height);
}

void DirCard::paintEvent(QPaintEvent *event)
{
    Q_D(DirCard);
    ...
    if (!d->_pCardPixmap.isNull()) {
        ...
        if (d->_pCardPixMode == ElaCardPixType::PixMode::Ellipse) {
            path.addEllipse(QPointF(d->_pCardPixmapSize.width() / 2 + width / 28, height() / 2),
                            d->_pCardPixmapSize.width() / 2,
                            d->_pCardPixmapSize.height() / 2);
            painter.setClipPath(path);
            painter.drawPixmap(QRect(width / 28 + d->_shadowBorderWidth,
            ...
            painter.drawPixmap(width / 28 + d->_shadowBorderWidth,
                               (height() - d->_pCardPixmapSize.height()) / 2,
                               d->_pCardPixmapSize.width(),
                               d->_pCardPixmapSize.height(),
                               d->_pCardPixmap);
        } else if (d->_pCardPixMode == ElaCardPixType::PixMode::RoundedRect) {
            path.addRoundedRect(QRectF(width / 28,
                                       (height() - d->_pCardPixmapSize.height()) / 2,
                                       d->_pCardPixmapSize.width(),
                                       d->_pCardPixmapSize.height()),
    ...
    painter.drawEllipse(QPointF(width * 0.95 + d->_shadowBorderWidth, height() * 0.25),
                        height() / 17,
                        height() / 17);
    painter.restore();
    ...
    painter.drawText(d->_pCardPixmapSize.width() + width / 11 + d->_shadowBorderWidth,
                     this->height() / 2 - d->_pTitleSpacing,
                     d->_pTitle);
    ...
    qreal rowTextWidth = (this->width() - this->width() / 11 - this->width() / 28
                          - d->_pCardPixmapSize.width() - 2 * d->_shadowBorderWidth);
    painter.drawText(QRect(d->_pCardPixmapSize.width() + width / 11 + d->_shadowBorderWidth,
                           height() / 2 + d->_pTitleSpacing,
                           rowTextWidth,
                           height() / 2),
